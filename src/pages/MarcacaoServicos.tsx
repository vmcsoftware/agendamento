...
export default function MarcacaoServicos() {
  ...
  useEffect(() => {
    getDocs(collection(db, "congregacoes"))
      .then((snap) => {
        const nomes = snap.docs.map((doc) => {
          const data = doc.data() as Record<string, unknown>;
          const nome = typeof data["nome"] === "string" ? (data["nome"] as string) : doc.id;
          return nome;
        });
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      toast({
        title: "Marcação de Serviço criada",
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-foreground">Marcação de Serviços</h1>
        <p className="text-muted-foreground mt-1">Cadastre serviços informando congregação, data e horário.</p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="pt-2">
              <Button type="submit" disabled={!form.formState.isValid}>Agendar Serviço</Button>
            </div>
          </form>
        </CardContent>
      </Card>
